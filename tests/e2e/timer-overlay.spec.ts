...
test('creates and closes a timer overlay window via the manager UI', async () => {
  ...
  try {
    ...
    const overlayWindowPromise = electronApp.waitForEvent('window', page => page !== mainWindow);
    ...
    const timerIdLine = await newTimer.locator('.timer-info p').first().textContent();
